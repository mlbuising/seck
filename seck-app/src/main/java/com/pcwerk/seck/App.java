...
import com.pcwerk.seck.classification.categorized.WekaCategoryClassifier;
...
public class App {
	...
	private void usage() {
		...
		System.out.println("classify-category-init 	initialize categories");
		System.out.println("classify-category-train 	display training data");
		System.out.println("classify-category		 		classify documents");
		...
		System.out
				.println("  --root-url=URL     		specify the crawling at root url");
		System.out
				.println("  --hbase-host=host  		specify the hostname (default=localhost");
		System.out
				.println("  --tc=#             		specify the thread count (default=1)");
		System.out
				.println("  --depth=#          		specify crawling depth (default = 1");
		System.out
				.println("  --file=FILENAME    		specify a datafile to save (if not specified, use to stdout)");
	}
	...
	private void run() {
		for (String command : commandStack) {
			...
			if (command.equals("crawl")) {
				...
				crawlInfo();
			} else if (command.equals("classify-category-init")) {
				if (!params.containsKey("root-url")) {
					usage();
					System.exit(0);
				}
				classifyCategoryInit();
			} else if (command.equals("classify-category")) {
				if (!params.containsKey("path")) {
					usage();
					System.exit(0);
				}
				classifyCategory();
			} else if (command.equals("classify-category-train")) {
				if (!params.containsKey("path")) {
					usage();
					System.exit(0);
				}
				classifyCategoryTrain();
	...
	private void classifyCategoryTrain() {
		Classifier classifier = new WekaCategoryClassifier();
		try {
			classifier.setTrainingData(params.get("path"));
		} catch (Exception e) {
			System.out.println("[i]    Processing training data unsuccessful.");
		}
		
	}

	private void classifyCategoryInit() {
		Classifier classifier = new WekaCategoryClassifier();
		classifier.initCategories(params.get("root-url"));
	}

	private void classifyCategory() {
		Classifier classifier = new WekaCategoryClassifier();
		try {
			classifier.classify(params.get("path"), params.get("root-url"));
		} catch (Exception e) {
			System.out.println("[i]    Classification data unsuccessful.");
		}
	}
